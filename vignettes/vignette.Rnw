...
\author{
Marco Antoniotti\footnote{Dipartimento di Informatica Sistemistica e Comunicazione, Università degli Studi Milano Bicocca
Milano, Italy.} \and
...
\begin{itemize}
\item[2.4] Current version. New statistics available for model confidence via cross-validation routines. New algorithms based on Minimum Spanning Tree extraction.
...
MST (Chow Liu) & Undirected Minimum Spanning Tree with Likelihood-Fit & Ind & \href{https://sites.google.com/site/troncopackage/}{In preparation.}\\ \hline
...
Function {\tt annotate.description} raises a warning if the dataset was previously annotated.
...
By default {\tt oncoprint}  will try to  sort samples and events to enhance exclusivity patterns among the events.
...
\begin{mybox}
<<oncocl, fig=TRUE, include=FALSE, width=5, height=5>>=
oncoprint(aCML, 
 	legend = FALSE, 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.5\textwidth, page=2]{vignette-onco}
\includegraphics[width=0.45\textwidth, page=2]{vignette-oncocl}
\end{center}
...
If one is willing to display samples grouped according to some variable, for instance after a sample clustering task, he can use {\tt group.samples} parameter of {\tt oncoprint} and that will override the mutual exclusivity ordering. Here, we make the trick of using the stages as if they were such clustering result.

\begin{mybox}
<<onco-clus2, fig=TRUE, include=FALSE, width=6, height=5>>=
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.45\textwidth, page=2]{vignette-onco-stages}
\includegraphics[width=0.45\textwidth, page=2]{vignette-onco-clus2}
\end{center}
\vspace*{.05in}
...
\FloatBarrier
    ...
    \begin{mybox}
    <<onco-pathway, fig=TRUE, include=FALSE, width=6.5, height=2>>=
...
\begin{mybox}
<<onco-ex-sel, fig=TRUE, include=FALSE, width=7, height=5.5, results=hide>>=
library(gridExtra)
grid.arrange(
	oncoprint(as.alterations(aCML, new.color = 'brown3'), 
		cellheight=6, cellwidth=4, gtable = T, silent = T, font.row = 6)$gtable,
	oncoprint(dataset, cellheight=6, cellwidth=4, gtable = T, silent = T, font.row = 6)$gtable,	
	ncol = 1	
)
@
\end{mybox}
...
\end{center}
\caption{Multiple output from \texttt{oncoprint} can be captured as a {\tt gtable} and composed via {\tt grid.arrange} (package {\tt gridExtra}). In this case we show  aCML data on top -- displayed after the {\tt as.alterations} transformation -- versus a selected subdataset of events with a minimum frequency of 5\%, force exclusion of SETBP1 (all events associated), and inclusion of EZH1 and EZH2.}
\end{figure*}
...
\subsection{CAPRI}
 ...
 In what follows, we show CAPRI's functioning by replicating the aCML case study presented in CAPRI's original paper. Regardless from which types of mutations we include, we select only the genes  mutated at least in the $5\%$ of the patients -- thus   we first use {\tt as.alterations} to have gene-level frequencies, and then we apply there a frequency filter (R's output is omitted). 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth, page=2]{vignette-onco-edited}
\end{center}
\vspace*{.05in}
...
\begin{mybox}
<<onco-kras-nras, fig=TRUE, include=FALSE, width=6, height=1>>=
oncoprint(events.selection(aCML.hypo, filter.in.names = c('KRAS', 'NRAS')), font.row = 8, ann.hits = FALSE)
@
\end{mybox}
...
\end{center}
which we can visualize via {\tt an oncoprint}.

\begin{mybox}
<<onco-tet2-idh2, fig=TRUE, include=FALSE, width=7, height=2,results=hide>>=
oncoprint(events.selection(aCML.hypo, filter.in.names = c('TET2', 'IDH2')), font.row = 8, ann.hits = FALSE)
@
\end{mybox}
...
\includegraphics[width=0.8\textwidth, page=2]{vignette-onco-tet2-idh2}
\end{center}
\vspace*{.05in}
...
\begin{mybox}
<<pattern-plot, fig=TRUE,include=FALSE, width = 4, height = 2.2>>=
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{vignette-pattern-plot}
\end{center}
...
\begin{mybox}
<<pattern-plot-circos, fig=TRUE,include=FALSE,results=hide,  width = 6, height = 6>>=
tronco.pattern.plot(aCML,
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{vignette-pattern-plot-circos}
\end{center}
...
\begin{mybox}
<<capri-plot, fig=TRUE,include=FALSE,width=4,height=4>>=
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{vignette-capri-plot}
\end{center}
...
\begin{mybox}
<<mst-plot, fig=TRUE,include=FALSE,results=hide,width=7,height=7>>=
par(mfrow=c(2,2))
tronco.plot(model.caprese, fontsize = 25, scale.nodes = 0.6, legend = F)
tronco.plot(model.edmonds, fontsize = 25, scale.nodes = 0.6, legend = F)
tronco.plot(model.chowliu, fontsize = 25, scale.nodes = 0.6, legend.cex = .7)
tronco.plot(model.prim, fontsize = 25, scale.nodes = 0.6, legend = F)
@
\end{mybox}

\begin{figure*}[ht]
\begin{center}
\includegraphics[width=1\textwidth]{vignette-mst-plot}
\end{center}
...
\subsection{Confidence via non-parametric, parametric and statistical bootstrap}
...
Bootstrap scores can be annotated to the {\tt tronco.plot} output by setting them via the confidence parameter {\tt confidence=c('npb', 'sb')}. In this case edge thickness will be proportional to the non-parametric ({\tt npb}) scores -- the last to appear in the {\tt confidence} parameter.
\begin{mybox}
<<figplotboot, fig=TRUE,include=FALSE,width=4,height=4>>=
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{vignette-figplotboot}
\end{center}
...
If we want to access a matrix with the scores and visualize that in a heatmap we can use for instance the {\tt pheatmap} function of TRONCO. In this case we need to use also function {\tt keysToNames} to translate internal TRONCO keys to mnemonic names in the plot 

\begin{mybox}
<<hboot, fig=TRUE,include=FALSE,width=7,height=7>>=
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.7\textwidth, page=2]{vignette-hboot} 
\end{center}
\vspace*{.05in}
...
\begin{mybox}
<<plot-conf, fig=TRUE,include=FALSE,width=4,height=4>>=
tronco.plot(model.boot, 
	fontsize = 12, 
	scale.nodes = .6, 
...
\begin{figure*}[ht]
\begin{center}
\includegraphics[width=0.8\textwidth]{vignette-plot-conf}
\end{center}
...
\section{Import/export with other tools}
...
\begin{enumerate}
\item \href{http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3866081/}{Network Based Stratification (NBS)},  a method for stratification (clustering) of patients in a cancer cohort based on genome scale somatic mutations measurements and a gene interaction network. You can export a TRONCO object in the NBS input format with  function {\tt export.nbs.input}, clustering outputs can be handled with standard TRONCO functions.
\item \href{http://www.ncbi.nlm.nih.gov/pubmed/25887147}{MUTEX}, a method for the identification of sets of mutually exclusive gene alterations in a given set of genomic profiles by scanning the groups of genes with a common downstream effect on the signaling network. You can export a TRONCO object in the MUTEX input format with  function {\tt export.mutex}, and output results can be imported with function {\tt import.mutex.groups}.
\end{enumerate}
