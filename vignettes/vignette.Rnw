...
\begin{itemize}
\item[2.4.2] Latest development version. Implements a noise model and finalizes a series of algorithms reconstructing Suppes-Bayes Causal Network as maximum spanning trees.
\item[2.4] Current stable version. New statistics available for model confidence via cross-validation routines. New algorithms based on Minimum Spanning Tree extraction.
...
MST (Gabow) & Partially Directed Minimum Spanning Tree with Mutual Information & Ind & \href{https://sites.google.com/site/troncopackage/}{In preparation.}\\ \hline
...
<<gabow-plot>>=
model.gabow = tronco.mst.gabow(aCML.clean, nboot = 5, boot.seed = 12345)
