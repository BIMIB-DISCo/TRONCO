...
\section{Implemented Algorithms, pipelines and case-sudies}
...
Explain which data is supported. spiega annotate.description
...
Oncoprints are the most effective  data-visualization functions in TRONCO. These are heatmaps where .... {\bf SPIEGAZIONE SULLE MAPPE ETC}
...
CAPRI is the only algorithm in TRONCO that supports hypotheses via logical formulas. Every formula is transformed into a a CAPRI  pattern among events (see the original CAPRI paper). For every hypothesis/pattern it is possible  to specify against which possible target it should be tested; if this is not done, it is tested against all other events in the dataset but those which constitute the pattern.
...
\paragraph{Querying and manipulating CAPRI's patterns.}

{\bf SONO ARRIVATO FINO QUI CONTINUO DOPO}
