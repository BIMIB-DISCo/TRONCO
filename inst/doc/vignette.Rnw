...
\TRONCO{} ({\sc TR}{\em anslational} {\sc ONCO}{\em logy}) is a \textsc{R} package which collects algorithms to infer progression models from Bernoulli 0/1 profiles of genomic alterations across a tumor sample. 
...
We will start by loading \TRONCO{} in the \textsc{R} console along with an example \textit{"dataset"} that comes with the package. 
<<>>=
library(TRONCO)
data(aCML)
...
\paragraph{We then use the function \texttt{show} to get a short summary of the aCML dataset.}
<<>>=
show(aCML)
@

\paragraph{With the function \texttt{as.events}, we check the events in the dataset.}
<<>>=
as.events(aCML)
@
...
\paragraph{Now we take a look at the alterations of the gene \texttt{SETBP1} across the samples.}
<<>>=
as.gene(aCML, genes='SETBP1')
@

\paragraph{We consider a subset of all the genes in the dataset to be involved in patters based on the support we found in the literature. See the main \textit{CAPRI} paper for the reference.}
<<>>=
gene.hypotheses = c('KRAS', 'NRAS', 'IDH1', 'IDH2', 'TET2', 'SF3B1', 'ASXL1')
@

\paragraph{Regardless from which types of mutations we include, we select only the genes which appear alterated at least in the $5\%$ of the patients. Thus, we first transform the dataset into \textit{"Alteration"} (i.e., by collapsing all the event types for the same gene), and then we use only the select events from the original dataset.}
<<>>=
alterations = events.selection(as.alterations(aCML), filter.freq = .05)
@

\paragraph{We display a plot of the selected genes. Note that this plot has no title as name annotation is not copied by the function \texttt{events.selection}.}
<<>>=
...
\paragraph{We display a new oncoprint with this dataset where we annotate the genes in \texttt{gene.hypotheses} in order to identify them. The sample names are also shown.}
...
\paragraph{We now also add the hypotheses that are described in CAPRI's manuscript.}

Hypothesis of hard exclusivity (XOR) for NRAS/KRAS events (Mutation). This hypothesis is tested against all the events in the dataset. 
<<>>=
hypo = hypothesis.add(hypo, 'NRAS xor KRAS', XOR('NRAS', 'KRAS'))
@

Here we try to include also a soft exclusivity (OR) pattern but, since its \textit{"signature"} is the same of the hard one, it will not be included. The code below is expected to rise an error. 
<<eval=FALSE>>=
...
\paragraph{We now add possible homologous events.}
...
\paragraph{The final dataset that will be given as input to CAPRI is now shown.}
<<>>=
dummy = oncoprint(hypo,  gene.annot = list(priors= gene.hypotheses), sample.id = T)
@
<<include=FALSE,echo=FALSE>>=
